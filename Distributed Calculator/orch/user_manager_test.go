...
func ApiRequests(w http.ResponseWriter, r *http.Request) {
	db := NewDBConnection()
	defer db.Close()
	...
	store := sqlkv.New(db, "LoginAndUser")
	store.SetString(user.Login, user.Password)
	...
	// _, err = stmt.Exec(123123, user.Login, user.Password)
	// if err != nil {
...
func TestLoginHandler(t *testing.T) {
	...
	http.HandleFunc("/login", ApiRequests)
